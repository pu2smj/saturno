/*-----------------------------------------------------------------
   Copyright (c)
   Arquivo......: rtc.c
   descrição....: real time clock module
   partly based on code from STMircoelectronics, Peter Dannegger
-------------------------------------------------------------------*/
...
static uint32_t struct_to_counter( const RTC_t *t ){
   ...
   /*the date is with extra day (more one). then I did (result-2). the correct is (result-1)*/
...
/*--------------------------------------------------------------
   ...
   Input          : none
   Output         : None
   comment        : sets default time-stamp if RTC has not been initialized before
--------------------------------------------------------------*/
void RTC_Configuration(void){
   ...
   for(i=0;i<5000;i++) { ; }                            /* LSI clock stabilization time */

   if (BKP_ReadBackupRegister(BKP_DR1) != 0xA5A5) {
      /* Backup data register value is not correct or not yet
      ...
      RTC_WaitForLastTask();                            /* Wait until last write operation on RTC registers has finished */
      //((hour*60 + minute)*60 + sec)                   /* Set initial value */
      RTC_SetCounter( (uint32_t)(0) );                  /* here: 1st January 2000 00:00:00*/
      RTC_WaitForLastTask();                            /* Wait until last write operation on RTC registers has finished */
      BKP_WriteBackupRegister(BKP_DR1, 0xA5A5);
      PWR_BackupAccessCmd(DISABLE);                     /* Lock access to BKP Domain */
   }else{
      RTC_WaitForSynchro();                             /* Wait for RTC registers synchronization */
   }
}
